...
function ensure_site_schema($pdo) {
    ...
    // Таблицы игрового эмулятора (originally created by the game server).
    // Создаём здесь, т.к. на Vercel нет игрового сервера.
    $pdo->exec("CREATE TABLE IF NOT EXISTS accounts (
        ...
        staff_role VARCHAR(32) NOT NULL DEFAULT 'player',
        is_verified SMALLINT NOT NULL DEFAULT 0,
        is_banned_reports SMALLINT NOT NULL DEFAULT 0,
        credits BIGINT NOT NULL DEFAULT 0,
        gold BIGINT NOT NULL DEFAULT 0,
        free_xp BIGINT NOT NULL DEFAULT 0,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS dossier (
    ...
    // Создаём админа если нет ни одного аккаунта
    $admin_check = $pdo->query("SELECT COUNT(*) FROM accounts")->fetchColumn();
    if (intval($admin_check) === 0) {
        $admin_pass = md5('admin123');
        $pdo->exec("INSERT INTO accounts (username, normalized_name, email, password_hash, is_admin, staff_role, is_verified, created_at) VALUES ('admin', 'admin', 'admin@test.projectorion.top', '$admin_pass', 1, 'admin', 1, NOW())");
    } else {
        // Обновляем email админа если старый
        $pdo->exec("UPDATE accounts SET email = 'admin@test.projectorion.top' WHERE username = 'admin' AND (email IS NULL OR email = '' OR email = 'admin@local')");
    }
